...
import { getClients, clearSuccess, getSubcategories, getBrands, searchClient } from '../../store/actions/appactions';
...
const Users = () => {
  ...
  const onOpenmessage = (search) => {
    setBrand(search);
    setDetails(true);
  }


  const onOpendelete = (search) => {
    setBrand(search);
    setDeletes(true);
  }
  ...
  const onSubmit = (e) => {
    e.preventDefault();
    dispatch(searchClient(titlesearch))
        ...
        <CCard>
          ...
          <CRow>
                ...
                <CCol xs="4">
                  ...
                  <CInput style={{height: 35}} id="name" placeholder="Search product name" value={titlesearch} onChange={(e) => setTitlesearch(e.target.value)} /> 
                      <CInputGroupPrepend>
